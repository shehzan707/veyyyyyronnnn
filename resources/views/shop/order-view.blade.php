    ...
    @if(isset($order))
            ...
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div style="flex:1;">
                    ...
                    <div style="margin-bottom:4px;"><strong>Status:</strong> 
                        <span style="padding: 2px 8px; border-radius: 4px; background: #e8f4f8; color: #0066cc;">{{ $order->order_status ?? 'N/A' }}</span>
                    </div>
        ...
        <table style="min-width:700px; width:100%; border-collapse:separate; border-spacing:0; background:#fff; box-shadow:0 2px 8px #eee; border-radius:10px;">
            ...
            <tbody>
                @foreach($order->items as $item)
                    ...
                    @endphp
                    <tr style="border-bottom:1px solid #f1f1f1; opacity: {{ $rowOpacity }}; background: {{ $isCancelled ? '#f5f5f5' : 'transparent' }};">
                        <td style="padding:12px 10px; text-align:left;">
                            ...
                            @endphp
                            <a href="{{ $productUrl }}" style="display:inline-block; text-decoration:none; cursor:pointer;">
                                <img src="{{ $imgUrl }}" alt="{{ $item->product->name ?? $item->product_name }}" style="width:60px; height:60px; object-fit:cover; border-radius:8px; box-shadow:0 1px 4px #eee; background:#f8f8f8; transition:transform 0.3s ease;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">
                            </a>
                        </td>
                        <td style="padding:12px 10px;">
                            <div>{{ $item->product->name ?? $item->product_name ?? 'N/A' }}</div>
                            @if($isCancelled)
                                <div style="font-size: 12px; color: #dc3545; margin-top: 4px; font-weight: 600;">🚫 Cancelled</div>
                            @endif
                        </td>
                        <td style="padding:12px 10px; text-align:left;">{{ $item->size ?? 'N/A' }}</td>
                        ...
                        <td style="padding:12px 10px; text-align:center;">
                            @if($canCancel)
                                <button type="button" class="cancel-item-btn" data-item-id="{{ $item->id }}" data-product-name="{{ $item->product->name ?? $item->product_name }}" data-quantity="{{ $item->quantity }}" data-refund="₹{{ number_format($item->price * $item->quantity, 2) }}" style="background: #dc3545; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-size: 12px;">Cancel Item</button>
                            @else
                    ...
                    @if($isCancelled && $item->cancel_reason)
                        <tr style="border-bottom:1px solid #f1f1f1; background: #fff9f9;">
                            <td colspan="7" style="padding:8px 12px; font-size: 12px; color: #666;">
                                @if($item->cancelled_by_admin ?? false)
        ...
        <div style="margin-top: 40px; padding: 24px; background: #f8f9fa; border-radius: 8px; max-width: 500px; margin-left: auto; margin-right: auto;">
            ...
            @foreach($order->items as $item)
                ...
                @if($isCancelled)
                    <div style="display: flex; justify-content: space-between; margin-bottom: 12px; font-size: 14px; color: #999; text-decoration: line-through;">
                        <span>{{ substr($item->product->name ?? $item->product_name ?? 'N/A', 0, 35) }}{{ strlen($item->product->name ?? $item->product_name ?? 'N/A') > 35 ? '...' : '' }} × {{ $item->quantity }}</span>
            ...
            <div style="display: flex; justify-content: space-between; margin-bottom: 12px; font-size: 14px; color: #666;">
                <span>Subtotal</span>
                <span>₹{{ number_format($activeSubtotal, 2) }}</span>
            </div>
            ...
            @if($activeSubtotal > 0)
                <div style="display: flex; justify-content: space-between; margin-bottom: 12px; font-size: 14px; color: #666;">
                    ...
                    <span>₹{{ number_format($platformFee, 0) }}</span>
                </div>
            @endif
            ...
            @if($activeSubtotal > 0 && $order->coupon_code && $order->discount_amount > 0)
                <div style="display: flex; justify-content: space-between; margin-bottom: 16px; font-size: 14px; color: #666;">
            ...
            <div style="display: flex; justify-content: space-between; padding-top: 8px;">
                ...
                <span style="font-size: 18px; font-weight: 700;">₹{{ number_format($activeSubtotal + ($activeSubtotal > 0 ? $platformFee : 0) + ($activeSubtotal > 0 ? $shippingCost : 0) - ($activeSubtotal > 0 ? ($order->discount_amount ?? 0) : 0), 2) }}</span>
            </div>
        </div>
