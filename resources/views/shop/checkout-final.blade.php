            ...
            <div class="checkout-content">
                ...
                <div class="step-content active" data-step="bag">
                    ...
                    <div class="billing-section">
                        <div class="billing-row">
                            ...
                            <span class="billing-value" id="subtotalAmount">₹{{ number_format($subtotal, 2) }}</span>
                        </div>
                        ...
                        <div class="billing-row total">
                            ...
                            <span id="totalAmount">₹{{ number_format($subtotal + 27 + $shipping - $discount, 2) }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="step-content" data-step="shipping">
                    ...
                    <div class="shipping-form">
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="first_name" placeholder="John" required value="{{ old('first_name', Auth::check() ? Auth::user()->first_name : '') }}">
                                </div>
                                <div class="form-group">
                                    <label>Last Name <span style="color: #dc3545;">*</span></label>
                                    <input type="text" name="last_name" placeholder="Doe" required value="{{ old('last_name', Auth::check() ? Auth::user()->last_name : '') }}">
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group">
                                    <label>Email <span style="color: #dc3545;">*</span></label>
                                    <input type="email" name="email" placeholder="john@example.com" required value="{{ old('email', Auth::check() ? Auth::user()->email : '') }}">
                                </div>
                                <div class="form-group">
                                    <label>Mobile <span style="color: #dc3545;">*</span></label>
                                    <input type="tel" name="mobile" placeholder="+91 XXXXXXXXXX" required value="{{ old('mobile', Auth::check() ? Auth::user()->mobile : '') }}">
                                </div>
                            </div>
                        </div>
                                ...
                                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 15px;">
                                    @foreach($addresses as $address)
                                        <label style="cursor: pointer;">
                                            <input type="radio" name="address_id" value="{{ $address->id }}" @if($loop->first) checked @endif style="cursor: pointer; margin-top: 5px;" onchange="document.getElementById('add-new-address-form').style.display='none';">
                                            <div style="border: 2px solid #ddd; border-radius: 8px; padding: 16px; background: white; transition: all 0.3s; cursor: pointer; margin-top: 5px;"
                                                ...
                                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; gap: 8px;">
                                                    <strong style="font-size: 16px; color: #222; font-weight: 700;">{{ $address->name }}</strong>
                                                ...
                                                <div style="background: linear-gradient(135deg, #f0f4ff 0%, #f9fbff 100%); border-left: 4px solid #0066cc; padding: 12px; border-radius: 6px; margin-bottom: 12px;">
                                                    <div style="color: #333; font-size: 13px; line-height: 1.7; font-weight: 500;">
                                                        @if($address->address)
                                                            <div style="margin-bottom: 4px;">{{ $address->address }}</div>
                                                        @endif
                                                        @if($address->address_line_1)
                                                            <div style="margin-bottom: 4px;">{{ $address->address_line_1 }}</div>
                                                        @endif
                                                        @if($address->address_line_2)
                                                            <div style="margin-bottom: 6px;">{{ $address->address_line_2 }}</div>
                                                        @endif
                                                        <div style="color: #555; font-weight: 600;">{{ $address->city }}, {{ $address->state }} - {{ $address->pincode }}</div>
                                                        <div style="color: #666; font-size: 12px; margin-top: 4px;">{{ $address->country ?? 'India' }}</div>
                                                    </div>
                                                </div>

                                                <!-- Contact Info -->
                                                <div style="display: flex; flex-direction: column; gap: 6px; font-size: 12px;">
                                                    <div style="color: #0066cc; font-weight: 600;">📱 {{ $address->mobile }}</div>
                                                    @if($address->email)
                                                        <div style="color: #666;">✉️ {{ $address->email }}</div>
                                                    @endif
                                                </div>
                                            </div>
                                        </label>
                        ...
                        <!-- NEW ADDRESS FORM -->
                        <div id="add-new-address-form" class="form-section" style="@if($addresses->count() == 0) display: block; @else display: none; @endif margin-top: 20px; padding: 20px; background: #fafafa; border: 2px dashed #ddd; border-radius: 8px;">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="address" placeholder="House no., Building name" value="{{ old('address') }}">
                                </div>
                                <div class="form-group">
                                    <label>Apartment/Suite</label>
                                    <input type="text" name="address_line2" placeholder="Apartment, suite, etc." value="{{ old('address_line2') }}">
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group">
                            ...
                            <div class="form-group">
                                <label>Pincode</label>
                                <input type="text" name="pincode" placeholder="XXXXXX" value="{{ old('pincode') }}">
                            </div>
                        </div>
                ...
                <div class="order-summary">
                    ...
                    <div class="summary-items" id="summaryItems">
                        @foreach($cart as $item)
                            <div class="summary-item">
                                <div class="summary-item-name">{{ substr($item['name'], 0, 20) }}{{ strlen($item['name']) > 20 ? '...' : '' }} × {{ $item['quantity'] }}</div>
                    ...
                    <div class="summary-rows">
                        ...
                        <div class="summary-row total" style="border-top: 1px solid #ddd; padding-top: 10px; margin-top: 10px;">
                            <span>Total Amount</span>
                            <span id="sidebarTotal">₹{{ number_format($subtotal + 27 + $shipping - $discount, 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
